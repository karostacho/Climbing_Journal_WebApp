...
class SqlData():
    ...
    def get_rock_routes_of_user_by(self, user_id, column_to_sort, order, scale):
        rows = self.connector.execute_sql_query((f'''SELECT lead_climbing_routes.grade_index, lead_climbing_routes.route_name, rock_climbing_grades."{scale}", lead_climbing_routes.date, lead_climbing_routes.comment 
